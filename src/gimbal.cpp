...
//        turn_camera_on_deg(0, 0);
    ...
    private:
        ...
        bool turn_camera_on_deg(int16_t yaw_deg, int16_t pitch_deg) {
            SBGC_cmd_control_ext_t c = {0};
            c.mode[0] = SBGC_CONTROL_MODE_NO;
//            if (yaw_deg == 0 and pitch_deg == 0) {
//                SBGC_cmd_execute_menu_send(SBGC_MENU_HOME_POSITION, sbgc_parser);
//            }
//            if (pitch_deg == 0) c.mode[1] = SBGC_CONTROL_MODE_ANGLE;
//            else c.mode[1] = SBGC_CONTROL_MODE_ANGLE_REL_FRAME | SBGC_CONTROL_FLAG_AUTO_TASK;
            c.mode[1] = SBGC_CONTROL_MODE_ANGLE;
            c.mode[2] = SBGC_CONTROL_MODE_ANGLE;
//            if (yaw_deg == 0) c.mode[2] = SBGC_CONTROL_MODE_ANGLE;
//            else c.mode[2] = SBGC_CONTROL_MODE_ANGLE_REL_FRAME | SBGC_CONTROL_FLAG_AUTO_TASK;

            c.data[0].angle = 0;
            c.data[1].angle = pitch_deg / 0.02197265625;
            c.data[2].angle = yaw_deg / 0.02197265625;
        ...
        void callbackDynamicReconfigure(mrs_serial::GimbalParamsConfig &config, uint32_t level) {
            ...
            turn_camera_on_deg(static_cast<int16_t>(config.yaw_angle),
                               static_cast<int16_t>(config.pitch_angle));
        }
